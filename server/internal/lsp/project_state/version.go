...
func LoadStdLib(logger commonlog.Logger, version string, c3cLibPath string, onRebuilt func(symbols_table.UnitModules)) symbols_table.UnitModules {
	...
	if detectedVersion != "" && detectedVersion != version {
		logger.Warning("requested C3 version does not match detected c3c binary version", "requested", version, "detected", detectedVersion)
	...
	logger.Info("loading stdlib", "version", version)
	return stdlib.LoadStdlibWithBackgroundRefresh(logger, version, c3cLibPath, onRebuilt)
}
